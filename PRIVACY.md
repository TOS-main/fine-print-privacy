...
There is no advertising, and nothing is ever sold or shared with third parties for marketing. We use Google Analytics to collect basic, privacy-friendly usage statistics (like feature usage) to help us improve the extension.
...
• A request for a site's /terms, /privacy etc.: Sent when you visit a site to read the document and grade it. Received by that site only (the same request your browser makes when you click the link yourself).
• A request for an extension's public Web Store listing: Sent only when you are looking at that listing or paste its link, to show what the extension declares about your data before you install it. Received by Google's Chrome Web Store only.
• A request for the developer's own website: Sent at the same moment if their listing links to one, to grade the company's terms alongside the extension's. Received by that developer's site only.
• Anonymous usage events & feature clicks: Sent when using the extension to understand feature usage and improve performance. Received by Google Analytics.

That's the complete list. All analytics data is aggregated and contains no personal identifiers or URLs you visit. Nothing in any of those requests identifies you. They carry no account, no id, and no record of anywhere else you have been — they are ordinary requests for public pages, of the kind your browser makes whenever you open a link.
...
• Grades and findings for sites you've visited: Stored in chrome.storage.local with a 7-day expiry so reopening a site is instant instead of re-fetching.
• A short grade history per site (last 10): Stored in chrome.storage.local to show whether a site's terms got worse over time.
• Sites and keywords you chose to watch: Stored in chrome.storage.sync (your settings, synced by Chrome across your own devices).
• What Fine Print did on a site (cookies rejected, popups hidden, boxes unticked): Stored in chrome.storage.local so the report can show it, and so you can undo it.
• Extensions you chose to watch, and what they last declared: Stored in chrome.storage.local to tell you if one changes what it collects.
• A random install id: Stored in chrome.storage.local for diagnostics. Random, not derived from anything about you.
...
• Personal identifiers, IP addresses, or URLs of pages you visit via analytics.
...
Sharing
Your data is not sold, rented, or traded to advertisers or data brokers. We use Google Analytics solely to collect aggregated usage metrics. No personally identifiable information or site browsing history is shared.
